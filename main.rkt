...
(struct birdState (x y v-y) #:transparent)
...
(struct initial-state (birdState pipes variables abilities score) #:transparent)
...
(define (get-initial-state)
  (define b1 (birdState bird-x bird-initial-y 0))
  (define pipe1 (pipe (+ added-number (random random-threshold)) scene-width))
  (define var1 (variables initial-gravity initial-momentum initial-scroll-speed))
  (define ability1 (abilities #t #f))
  (initial-state b1 (list pipe1) var1 ability1 0))
...
(define (get-bird state)
  (initial-state-birdState state))

(define (get-bird-y birdUse)
  (birdState-y birdUse))

(define (get-bird-x birdUse)
  (birdState-x birdUse))
...
(define (next-state-bird bird gravity)
  (struct-copy birdState bird [y (+ (get-bird-y bird) (get-bird-v-y bird))] [v-y (+ (get-bird-v-y bird) gravity)]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy birdState bird [v-y (- 0 momentum)]))
...
(define (change current-state pressed-key)
    (match-let ([(initial-state birdState _ _ _ _) current-state])
    (cond [(key=? pressed-key " ") (struct-copy initial-state current-state [birdState (next-state-bird-onspace birdState initial-momentum)])]
          [else current-state])))
...
(define (get-pipes state)
  (initial-state-pipes state)) 


;TODO 10
; Implementați get-pipe-x ce va extrage dintr-o singură structura de tip pipe, x-ul acesteia.
(define(get-pipe-x pipe-to-return)
  (pipe-x pipe-to-return))


(define(get-pipe-gap-y pipe-to-return)
  (pipe-gap-y pipe-to-return))
...
; și scroll-speed(un număr real). Aceasta va scadea x-ului fiecărui pipe
; scroll-speed-ul dat.
(define (move-pipes pipesList scroll-speed)
    (map (λ (n) (struct-copy pipe n [x (- (get-pipe-x n) scroll-speed)])) pipesList)) 
...
(define (clean-pipes pipesList)
  (cond
    [(empty? pipesList) null]
    [(< (+ pipe-width (get-pipe-x (car pipesList))) 0) (clean-pipes (cdr pipesList))]
    [else (cons (car pipesList) (clean-pipes (cdr pipesList)))]))
...
(define (add-more-pipes pipesList)
  (if (< (length pipesList) no-pipes)
      (append pipesList (list (pipe (+ added-number (random random-threshold)) (+ (get-pipe-x (last-element pipesList)) (+ pipe-width pipe-gap))))) 
      pipesList))
...
(define (next-state-pipes pipesList scroll-speed)
  (move-pipes (clean-pipes (add-more-pipes pipesList)) scroll-speed))

;TODO 17
; Creați un getter ce va extrage scorul din starea jocului.
(define (get-score state)
  (initial-state-score state))
...
(define (check-ground-collision bird)
 (if (>= (+ (get-bird-y bird) bird-height) ground-y) #t
  #f))
...
(define (check-pipe-collisions bird pipes)
  (or (check-collision-rectangles (make-posn (get-bird-x bird) (get-bird-y bird)) ; A1
                              (make-posn (+ bird-width (get-bird-x bird)) (+ bird-height (get-bird-y bird))) ; A2
                              (make-posn (get-pipe-x (car pipes)) 0) ; B1
                              (make-posn (+ pipe-width (get-pipe-x (car pipes))) (get-pipe-gap-y (car pipes)))) ; B2
      
      (check-collision-rectangles (make-posn (get-bird-x bird) (get-bird-y bird)) ; A1
                              (make-posn (+ bird-width (get-bird-x bird)) (+ bird-height (get-bird-y bird))) ; A2
                              (make-posn (get-pipe-x (car pipes)) (+ pipe-self-gap (get-pipe-gap-y (car pipes)))) ; C1
                              (make-posn (+ pipe-width (get-pipe-x (car pipes))) scene-height)))) ; C2
...
(define (next-state state)
  (struct-copy initial-state state [birdState (next-state-bird (get-bird state) initial-gravity)]
...
(define gap-image (rectangle (add1 pipe-width) pipe-self-gap "solid" "white"))
...
(define (place-pipes pipes scene)
	(foldl (λ (elem img)
                 (place-image gap-image (+ (get-pipe-x elem) (quotient pipe-width 2)) (+ (get-pipe-gap-y elem) (quotient pipe-self-gap 2))
                     (place-image pipe-image (+ (get-pipe-x elem) (quotient pipe-width 2)) (quotient pipe-height 2) img))) scene pipes))


(define (draw-frame state)
  (match-let ([(initial-state birdState _ _ _ _) state])
  (place-image bird-image (+ (birdState-x birdState) (quotient bird-width 2)) (+ (birdState-y birdState) (quotient bird-height 2))
